...
def batch_processing_example():
    ...
    for i, (text, result) in enumerate(zip(explanations, results), 1):
        status = "✅" if result.confidence > 0.5 else "⚠️" if result.confidence > 0.3 else "❌"
...
def debugging_example():
    ...
    for landmark in debug_info['detected_landmarks']:
        print(f"  - {landmark['name']} ({landmark['type']}) - {landmark['confidence']:.2f}")
...
def error_handling_example():
    ...
    for text in problematic_cases:
        try:
            result = parser.parse(text)
            print(f"'{text}' → confidence: {result.confidence:.2f}, notes: {result.processing_notes}")
