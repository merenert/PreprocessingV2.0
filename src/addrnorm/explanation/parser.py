...
class ExplanationParser:
    ...
    def parse(self, explanation_text: str) -> ExplanationResult:
        ...
        # Initialize result
        result = ExplanationResult(raw_explanation=explanation_text, type="landmark")
        ...
        if landmarks:
            ...
            if best_landmark.confidence >= self.config.landmark_detection_threshold:
                ...
                self.logger.debug(
                    f"Detected landmark: {best_landmark.name} ({best_landmark.type})"
                )
            else:
...
def create_parser(
    debug_mode: bool = False, min_confidence: float = 0.3
) -> ExplanationParser:
    """
    ...
    config = ExplanationConfig(
        debug_mode=debug_mode, min_confidence_threshold=min_confidence
    )
...
def extract_landmark_info(text: str) -> Optional[Dict[str, str]]:
    ...
    if result.landmark and parser.is_valid_result(result):
        return {"name": result.landmark.name, "type": result.landmark.type}
