...
class ExplanationConfig(BaseModel):
    ...
    min_confidence_threshold: float = Field(
        default=0.3, description="Minimum confidence for results"
    )
    landmark_detection_threshold: float = Field(
        default=0.5, description="Landmark detection threshold"
    )
    relation_detection_threshold: float = Field(
        default=0.6, description="Relation detection threshold"
    )
    enable_fuzzy_matching: bool = Field(
        default=True, description="Enable fuzzy string matching"
    )
    max_tokens_per_landmark: int = Field(
        default=5, description="Max tokens in landmark name"
    )
