...
class MLPatternSuggester:
    ...
    def suggest_patterns(self, addresses: List[str], max_patterns: int = 10) -> List[PatternSuggestion]:
        ...
        quality_filtered = [p for p in ranked_patterns if p.confidence >= self.config.min_pattern_confidence]
    ...
    def _extract_pattern_from_cluster(self, cluster: AddressCluster) -> Optional[PatternSuggestion]:
        ...
        # Create proper models for PatternSuggestion
        cluster_id_int = int(cluster.cluster_id.split("_")[-1]) if "_" in cluster.cluster_id else 0

        cluster_result = ClusterResult(
            cluster_id=cluster_id_int,
            addresses=addresses,
            size=cluster.cluster_size,
            similarity_score=cluster.confidence_score,  # Use confidence as similarity
        )

        pattern_template = PatternTemplate(
            template=regex_pattern,  # Use the regex as template
            components=["mahalle", "sokak", "cadde", "no"],  # Common fields
            example=addresses[0] if addresses else "Example address",
            complexity_score=0.5,  # Fixed complexity for demo
            generalizability=cluster.confidence_score * 0.8,  # Estimate
        )

        return PatternSuggestion(
            pattern_id=pattern_id,
            pattern_type=PatternType.REGEX,  # Use existing type
            regex_pattern=regex_pattern,
            template=pattern_template,
            source_cluster=cluster_result,
            confidence=cluster.confidence_score,
            coverage=min(len(addresses) / 10.0, 1.0),  # Estimate coverage
            examples=addresses[:3],  # First 3 as samples
